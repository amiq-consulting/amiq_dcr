...
help() {
	...
	echo "	-dvt_debug             --> enable debugging directly from DVT"
...
run_with_ius_test() {
	...
	if [ "$run_mode" = "interactive" ]; then
		...
		echo "simvision input  ${EXAMPLE_DIR}/scripts/amiq_dcr.svcf" >> ncsim_cmds.tcl
...
while [ $# -gt 0 ]; do
	case `echo $1 | tr "[A-Z]" "[a-z]"` in
        ...
        ;;
      	...
      	;;
      	-dvt_debug)
      		EXTRA_OPTIONS=" ${EXTRA_OPTIONS}  -f ${PROJECT_DIR}/examples/common/scripts/dvt_debug.f "
      	;;
    esac
    shift
done
